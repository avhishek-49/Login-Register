...
router.get('/', (req, res, next) => {
  if(req.session.flag == 1) {
    req.session.destroy();
    res.render('index', { title: 'Smudge', message : 'Email Already Exists.', flag : 1 });
  }
  else if(req.session.flag == 2) {
    req.session.destroy();
    res.render('index', { title: 'Smudge', message : 'Registration Successfully Completed. Please login!', flag : 0 });
  }
  else if(req.session.flag == 3) {
    req.session.destroy();
    res.render('index', { title: 'Smudge', message : 'Confirm Password Does not Match.', flag : 1 });
  }
  else if(req.session.flag == 4) {
    req.session.destroy();
    res.render('index', { title: 'Smudge', message : 'Incorrect Email OR Password.', flag : 1 });
  }
  else if(req.session.flag == 5) {
    req.session.destroy();
    res.render('index', { title: 'Smudge', message : 'Please Login your Account.', flag : 1 });
  }
  else if(req.session.flag == 6) {
    req.session.destroy();
    res.render('index', { title: 'Smudge', message : "Form Validation Error", flag : 1 });
  }
  else {
    res.render('index', { title: 'Smudge'});
  }
});
